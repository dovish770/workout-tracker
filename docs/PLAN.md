...
- [ ] **הוצאת המסך מתוך ה־AppShell** דרך route groups: `app/(main)/` לכל מה שעטוף בהדר, `app/(focus)/` למסך האימון. הכתובות לא משתנות
- [ ] `ToastProvider` עולה ל־`app/layout.tsx`, כדי ששני העולמות יקבלו הודעות
- [ ] מכונת מצבים בשלבים, כל אחד ממלא מסך:
  ...
  - `done` — "סיימת. כל הכבוד."
- [ ] פעולה ראשית אחת בכל מסך, כפתור גדול — ללחיצה עם יד מיוזעת, לא לעכבר
- [ ] ביטול הסט האחרון תמיד זמין, אבל משני
- [ ] רשימת התרגילים הבאים מכווצת, לא על המסך הראשי

**8ד · טיימר מנוחה**

- [ ] `useRestTimer` — נספר מ־**חותמת זמן התחלה**, לא מצבירת טיקים. הטלפון ננעל באמצע מנוחה, ו־`setInterval` נחנק או נעצר ברקע; חישוב מ־timestamp נשאר מדויק גם אחרי שהמסך חוזר
- [ ] המשתמש בוחר אם להפעיל טיימר בכלל, ובנפרד אם להציג אותו
- [ ] איפוס הטיימר הנוכחי להתחלה
- [ ] כשנגמר: **שינוי צבע**, ומעבר לספירה **למעלה** — כמה שניות עברו מאז שנגמר
- [ ] המצב נשמר בין סטים ב־`localStorage` (העדפה), לא ב־DB

**8ה · עדכון משקל שיא תוך כדי אימון**

- [ ] הכותרת הקבועה של מסך הסט היא גם נקודת העריכה — הקשה עליה פותחת קלט מספרי
- [ ] `updateSessionExerciseMaxWeight` — כותב **גם** ל־`session_exercises` וגם ל־`exercises` של האימון המקורי, ב־batch אחד
- [ ] זו הכתיבה היחידה שחוצה במכוון את קו הצילום: כאן זו בדיוק הכוונה — שיא חדש הוא עובדה חדשה על התרגיל, לא רק על האימון הנוכחי
- [ ] אם התרגיל המקורי נמחק בינתיים — רק הצילום מתעדכן, בלי שגיאה
