...
- [ ] טבלת `exercises`: `id`, `workout_id`, `position`, `name`, `sets`, `reps`, `max_weight`, `created_at`, `updated_at`
- [ ] `workout_id` → `references workouts(id) on delete cascade` — מחיקת אימון מוחקת את תרגיליו ברמת ה־DB, לא בקוד
- [ ] אינדקס על `(workout_id, position)` — **בלי `unique`**: סידור מחדש עובר דרך מצבי ביניים שבהם שני תרגילים חולקים position, ואילוץ ייחודיות היה נכשל באמצע
- [ ] `relations()` ב־`db/schema.ts` כדי לאפשר `with: { exercises }`
- [ ] מיגרציה בשני חלקים: DDL אוטומטי מ־drizzle-kit, ואז SQL **ידני** שמעביר את ה־jsonb הקיים לשורות (`jsonb_array_elements` עם `ordinality` בשביל ה־position)
- [ ] אימות שמספר השורות שנוצרו שווה למספר האיברים ב־jsonb — ורק אז `DROP COLUMN exercises`

**5ב · כתיבה**

- [ ] אסטרטגיית שמירה: מחיקת כל תרגילי האימון והכנסה מחדש, עם ה־ids שהגיעו מהטופס. פשוט, אטומי, ומשמר ids של תרגילים קיימים
- [ ] `position` נגזר מהאינדקס במערך — הסדר עדיין נקבע בטופס, רק מאוחסן אחרת
- [ ] הכל בתוך `db.batch()`: הדרייבר של Neon מעל HTTP **לא תומך** ב־`db.transaction()` אינטראקטיבי, אבל batch רץ כטרנזקציה אחת ב־round trip אחד
- [ ] אם בעתיד נצטרך טרנזקציות אינטראקטיביות — מעבר ל־`drizzle-orm/neon-serverless` עם `Pool` מעל WebSocket. לא נדרש עכשיו

**5ג · קריאה**

- [ ] `list()` מפסיק להביא תרגילים בכלל — `count()` ב־left join. שיפור אמיתי: דף הרשימה כבר לא מוריד את כל התרגילים של כל האימונים
- [ ] טיפוס `WorkoutSummary` (עם `exerciseCount`) לרשימה, מול `Workout` (עם `exercises`) לדף הפרטים
- [ ] `getById` עם `with: { exercises: { orderBy: position } }`

**5ד · מה לא אמור להשתנות**

- [ ] אפס שינויים תחת `app/` ו־`components/` — זו הבדיקה האמיתית לגבול ה־repository שבנינו בשלב 2
- [ ] הסכימות של zod, הטופס והגרירה נשארים כמו שהם
- [ ] עדכון §7 בקובץ הקונבנציות, שכרגע אומר במפורש "jsonb ולא טבלה שנייה"
